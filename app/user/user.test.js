...
describe('User model', () => {
  ...
  test('User fetch by id', async () => {
    ...
    try {
      const user = await new User({
        userId: '2272e61b-59b5-4da3-9ab6-27c804930ca5',
      }).fetch();
      expect(user.get('userId')).toEqual(
        '2272e61b-59b5-4da3-9ab6-27c804930ca5',
      );
      expect(user.get('firstname')).toBe('Member');
  ...
  test('User fetch id 8db9c992-f0f2-43ef-9264-4a20540e3bcf should fail', () =>
    expect(
      new User({ userId: '8db9c992-f0f2-43ef-9264-4a20540e3bcf' }).fetch(),
  ...
  test('User should not update role not available', async () => {
    expect.assertions(1);
    const user = new User({
      userId: 'abf65ce3-1a02-475a-a434-dcda9131f068',
    }).fetch();
    try {
      user.role = 'enabled';
      await user.save();
